...
private:
    void initializeROS(ros::NodeHandle& nh, const std::string& uav_name) {
        octomap_sub = nh.subscribe<octomap_msgs::Octomap>(uav_name + "/octomap_full", 1, std::bind(&UAV::octomapCallback, this, std::placeholders::_1));
    ...
    void frontierDetection()
    {
        ...
        for(octomap::OcTree::iterator it = octree->begin(d_exp), end=octree->end(); it!= end; ++it)
        {
            ...
            if (current_node == nullptr || octree->isNodeOccupied(current_node))
                continue;
    ...
    void getNeighbor(const octomap::OcTreeKey& key, std::vector<octomap::point3d>& neighbors) 
    {
        ...
        int depth_diff = std::pow(2, octree->getTreeDepth() - d_exp);
    ...
    void frontierEvaluation()
    {
        ...
        for (auto iter = candidates.begin(), end = candidates.end(); iter != end; ++iter) {
            ...
            // Calculate the distance from the current UAV position to the candidate
            double distance = calculateDistance(pos, candidate);
            
            // Calculate the sum of distances to other UAVs and check if candidate should be skipped
            ...
            // Calculate information gain and utility score
        ...
        // Set the waypoint based on whether a best candidate was found
        if (best_candidate == octomap::point3d(0, 0, 0)) {
            waypoint = pos;
            waypoint_score = 0;
        } else {
        ...
        for(double dx = bbx_min.x(); dx < bbx_max.x(); dx += r_exp) {
            for(double dy = bbx_min.y(); dy < bbx_max.y(); dy += r_exp) {
                for (double dz = bbx_min.z(); dz < bbx_max.z(); dz += r_exp) {
                    ...
                    {
                        total++;
                        if(!octree->search(dx, dy, dz) && isWithinExplorationBounds(octomap::point3d(dx, dy, dz)))
                            unknown++;
                    }
                }
            }
